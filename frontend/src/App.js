  ...
  return (
    <div style={{ backgroundColor: '#0f172a', minHeight: '100vh', padding: '20px', color: 'white', fontFamily: 'Arial' }}>
      ...
      <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '30px' }}>
        {stats.map((city, i) => (
          <div key={city.city} style={{ backgroundColor: '#1e293b', borderRadius: '12px', padding: '20px', minWidth: '180px', textAlign: 'center', border: `1px solid ${COLORS[i]}` }}>
      ...
      <div style={{ backgroundColor: '#1e293b', borderRadius: '12px', padding: '20px', marginBottom: '30px' }}>
        ...
        <svg viewBox="0 0 500 500" style={{ width: '100%', height: '350px', backgroundColor: '#0f172a', borderRadius: '8px' }}>
          <ellipse cx="250" cy="270" rx="180" ry="200" fill="none" stroke="#334155" strokeWidth="1" />
          {stats.map((city, i) => {
            const pos = CITY_POSITIONS[city.city];
            if (!pos) return null;
            return (
              <g key={city.city}>
                <circle cx={pos.x} cy={pos.y} r={15} fill={COLORS[i]} opacity={0.8} />
                <circle cx={pos.x} cy={pos.y} r={20} fill="none" stroke={COLORS[i]} strokeWidth="2" opacity={0.4} />
                <text x={pos.x} y={pos.y - 25} textAnchor="middle" fill="white" fontSize="13" fontWeight="bold">{city.city}</text>
                <text x={pos.x} y={pos.y + 35} textAnchor="middle" fill={COLORS[i]} fontSize="11">{city.total_events.toLocaleString()}</text>
              </g>
            );
          })}
        </svg>
